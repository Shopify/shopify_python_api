...
class PaginationTest(TestCase):
    ...
    def test_nonpaginates_collection(self):
        ...
        self.assertIsInstance(
            draft_orders,
            shopify.collection.PaginatedCollection,
    ...
    def test_pagination_next_page(self):
        ...
        n = c.next_page()
        self.assertEqual(
            n.previous_page_url, self.prev_page_url, "prev url is incorrect"
        )
        self.assertIsInstance(
            n,
            shopify.collection.PaginatedCollection,
        ...
        self.assertIn(
            "previous",
            n.metadata["pagination"],
            "next_page() collection doesn't have a previous page",
        )

        with self.assertRaises(
            IndexError, msg="next_page() did not raise with no next page"
        ):
            n.next_page()
    ...
    def test_pagination_previous(self):
        ...
        self.assertEqual(
            len(p), 4, "previous_page() collection has incorrect length"  # cached
        )
        ...
        with self.assertRaises(
            IndexError, msg="previous_page() did not raise with no previous page"
        ):
            p.previous_page()
