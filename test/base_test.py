...
class BaseTest(TestCase):
    ...
    def test_activate_session_should_set_site_and_headers_for_given_session(self):
        ...
        self.assertEqual(
            "https://shop1.myshopify.com/admin/api/unstable", shopify.Shop.site
        )
        self.assertIsNone(ActiveResource.headers)
        self.assertEqual(
            "token1", shopify.ShopifyResource.headers["X-Shopify-Access-Token"]
        )
    ...
    def test_activate_session_should_set_site_given_version(self):
        ...
        self.assertEqual(
            "https://shop2.myshopify.com/admin/api/2019-04", shopify.Shop.site
        )
        self.assertIsNone(ActiveResource.headers)
    ...
    def test_activate_session_with_one_session_then_clearing_and_activating_with_another_session(
        ...
        self.assertIsNone(ActiveResource.headers)
        self.assertEqual(
            "token2", shopify.ShopifyResource.headers["X-Shopify-Access-Token"]
        )
    ...
    def test_headers_includes_user_agent(self):
        ...
        t = threading.Thread(
            target=lambda: self.assertTrue(
                "User-Agent" in shopify.ShopifyResource.headers
            )
        )
        t.start()
        t.join()
    ...
    def test_headers_is_thread_safe(self):
        ...
        t2 = threading.Thread(
            target=lambda: self.assertFalse(
                "X-Custom" in shopify.ShopifyResource.headers
            )
        )
        t2.start()
        t2.join()
    ...
    def test_setting_with_user_and_pass_strips_them(self):
        ...
        API_KEY = "user"
        PASSWORD = "pass"
        shop_url = "https://%s:%s@this-is-my-test-show.myshopify.com/admin" % (
            API_KEY,
            PASSWORD,
        )
