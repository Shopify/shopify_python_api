...
class InventoryLevelTest(TestCase):
    ...
    def test_fetch_inventory_level(self):
        ...
        self.fake(
            ...
            "?location_ids=905684977%2C487838322"
            "&inventory_item_ids=808950810%2C39072856",
            method='GET',
            extension='',
        ...
        self.assertTrue(
            all(
                item.location_id in params['location_ids'] and
                item.inventory_item_id in params['inventory_item_ids'] for
                item in inventory_levels)
        )
    ...
    def test_destroy_inventory_level(self):
        inventory_level_response = json.loads(
            self.load_fixture('inventory_level').decode()
        )
        inventory_level = shopify.InventoryLevel(
            inventory_level_response['inventory_level'])
