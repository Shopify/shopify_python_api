...
class CustomerTest(TestCase):
    def setUp(self):
        ...
        self.fake(
            "customers/207119551", method="GET", body=self.load_fixture("customer")
        )
    ...
    def test_send_invite_with_params(self):
        ...
        customer_invite_response = self.customer.send_invite(
            shopify.CustomerInvite(customer_invite["customer_invite"])
        )
        self.assertEqual(
            customer_invite, json.loads(self.http.request.data.decode("utf-8"))
        )
        self.assertIsInstance(customer_invite_response, shopify.CustomerInvite)
        self.assertEqual(
            customer_invite["customer_invite"]["to"], customer_invite_response.to
