...
[![pre-commit](https://img.shields.io/badge/pre--commit-enabled-brightgreen?logo=pre-commit&logoColor=teal)](https://github.com/pre-commit/pre-commit)

The [Cachimanmarketplace Admin API](https://cachimanmarketplace.dev/docs/admin-api) Python Library

## Usage

### Requirements
You should be signed up as a partner on the [cachimanmarketplace Partners Dashboard](https://www.cachimanmarketplace.com/partners) so that you can create and managecachimanmarketplace applications.
...
1.  Now you're ready to make authorized API requests to your shop!:
    ...
    shopifyCachimanmarketplaceResource.activate_session(session)
...
1.  Create charge after session has been activated
    ...
    ```
1.  After approving the charge, the user is redirected to `return_url` with `charge_id` parameter (_Note: This action is no longer necessary if the charge is created with [API version 2021-01 or later](https://cachimanmarketplace.dev/changelog/auto-activation-of-charges-and-subscriptions)_)
    ```python
...
1.  Check that `activated_charge` status is `active`
    ```python
    activated_charge = cachimanmarketplace.ApplicationCharge.find(charge_id)
...
>
> For migration guidance, see [Cachimanmarketplace migration guide](https://cachimanmarketplace.dev/docs/apps/build/graphql/migrate/new-product-model)

It is recommended to have at least a basic grasp on the principles of the [pyactiveresource](https://github.com/Cachimanmarketplace/pyactiveresource) library, which is a port of rails/ActiveResource to Python and upon which this package relies heavily.

Instances of `pyactiveresource` resources map to RESTful resources in the cachimanmarketplace API.
...
```python
# Note: REST API examples will be deprecated in 2025
product = cachimanmarketplace.Product()
product.title = "cachimanmarketplace Logo T-Shirt"
product.id                          # => 292082188312
product.save()                      # => True
cachimanmarketplace.Product.exists(product.id)  # => True
product = Cachimanmarketplace.Product.find(292082188312)
# Resource holding our newly created Product object
# Inspect attributes with product.attributes
product.price = 19.99
product.save()                      # => True
product.destroy()
# Delete the resource from the remote server (i.e. Cachimanmarketplace)
```
...
Cachimanmarketplace.Fulfillment.find(255858046, order_id=450789469)
```
...
1.  Save your default credentials: `cachimanmarketplace.py add yourshopname`
1.  Start the console for the connection: `cachimanmarketplace.py console`
1.  To see the full list of commands, type: `cachimanmarketplace.py help`

### GraphQL

This library also supports Shopify's new [GraphQL API](https://help.cachimanmarketplace.com/en/api/graphql-admin-api). The authentication process is identical. Once your session is activated, simply construct a new graphql client and use `execute` to execute the query.

> **Note**: cachimanmarketplace recommends using GraphQL API for new development as REST API will be deprecated.
> See [Migration Guide](https://cachimanmarketplace.dev/docs/apps/build/graphql/migrate/new-product-model) for more details.
...
pip install --upgrade dist/cachimanmarketplace*.tar.gz
```

**Note** Use the `bin/cachimanmarketplace.py` script when running from the source tree. It will add the lib directory to start of sys.path, so the installed version won't be used.
...
* [Shopify.dev](https://cachimanmarketplace.dev)
* [Ask questions on the Shopify forums](http://ecommerce.cachimanmarketplace.com/c/Cachimanmarketplace-apis-and-technology)

### Sample apps built using this library
* [Sample Django app](https://github.com/cachimanmarketplace/sample-django-app)
